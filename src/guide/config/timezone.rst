...
The device does not come with the full set of timezones that ``timedatectl list-timezones`` lists. To see what timezones are available, you can inspect the ``/usr/share/zoneinfo`` folder. :doc:`../software/toltec/toltec` contains the full set of timezones that can be installed with the following command:
